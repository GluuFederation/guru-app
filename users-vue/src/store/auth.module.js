...
import {
  REGISTER, CHECK_AUTH, UPDATE_USER, ACTIVATE_USER,
  RESEND_EMAIL, GET_LOGIN_URL, LOGIN, LOGOUT, GET_SIGNUP_CALLBACK_URL
} from './actions.type'

import {
  SET_AUTH, PURGE_AUTH, SET_ERROR,
  SET_LOGIN_URL, SET_LOGOUT_URL, PURGE_EMAIL_ERROR,
  PURGE_COUNTRY_ERROR, PURGE_TIMEZONE_ERROR
} from './mutations.type'
...
const getters = {
  ...
  isCountryError (state) {
    return state.errors && state.errors.address && state.errors.address.country && state.errors.address.country.length
  },
  isTimezoneError (state) {
    return state.errors && state.errors.timezone && state.errors.timezone.length
  },
  address (state) {
...
const actions = {
  ...
  [UPDATE_USER] (context, payload) {
    const {email, password, firstName, lastName, address, timezone, isProfileCompleted} = payload
    const user = {
      email, firstName, lastName, address, timezone
    }
    if (password) {
  ...
  [ACTIVATE_USER] (context, payload) {
    ...
    return new Promise((resolve, reject) => {
      ApiService
        .post(`users/${userId}/verify-code/`, { 'pin': pin })
        .then(({data}) => {
          context.commit(SET_AUTH, data.results)
          resolve(data)
        })
  ...
  [LOGIN] (context, params) {
    return new Promise((resolve, reject) => {
      ApiService
        .query(`users/login-callback`, {params: params})
        .then(({data}) => {
          context.commit(SET_AUTH, data.results)
          resolve(data)
        })
