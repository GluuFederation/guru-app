                ...
                <b-card class="card-gluu">
                  ...
                  <b-card-body>
                    <b-form-group>
                      ...
                      <div>
                        <gluu-tag v-for="(item, key) in company" :key="key" :tag="item" v-on:clear="removeCompanyTag"></gluu-tag>
                      </div>
                      <multi-select
                    ...
                    <b-form-group>
                      ...
                      <div>
                        <gluu-tag v-for="(item, key) in createdBy" :key="key" :tag="item" v-on:clear="removeAuthorTag"></gluu-tag>
                      </div>
                      <multi-select
                    ...
                    <b-form-group>
                      ...
                      <div>
                        <gluu-tag v-for="(item, key) in assignedBy" :key="key" :tag="item" v-on:clear="removeAssignedByTag"></gluu-tag>
                      </div>
                      <multi-select
                        class="select-tags-gluu single-select"
                        v-model="assignedBy"
                  ...
                  <b-row>
                    <b-col cols="12">
                      <div id="tags-container" class="d-flex">
                        ...
                        <div class="ml-auto clear-all-btn"><a v-on:click="clearAllTags">Clear all</a></div>
                      </div>
                    </b-col>
                  </b-row>
                </div>
...
export default {
    ...
    return {
      tickets: [
        ...
        },
        {
          id: 2,
          title: 'Admin account locked out',
          slug: 'admin-account-locked-out2',
          status: 'pending',
          issueType: 'minor',
  ...
  methods: {
    ...
    addCategoryTag (tag) { this.addConfigTag(tag, 'category') } ,
    removeCategoryTag (tag) { this.removeConfigTag(tag, 'category') },
    
    addProductTag (tag) { this.addConfigTag(tag, 'product') },
    removeProductTag (tag) { this.removeConfigTag(tag, 'product') },

    addIssueTypeTag (tag) { this.addConfigTag(tag, 'issueType') },
    removeIssueTypeTag (tag) { this.removeConfigTag(tag, 'issueType') },

    addStatusTag (tag) { this.addConfigTag(tag, 'status') },
    removeStatusTag (tag) { this.removeConfigTag(tag, 'status') },
    ...
    removeConfigTag (tag, configFilterType=null) {
      ...
      if (configFilterType === null) {
        ...
        switch(tag.configFilterType) {
          case 'category':
            this.category.splice(this.category.findIndex(item => item.slug === tag.slug), 1)
            break
          case 'product':
            this.product.splice(this.product.findIndex(item => item.slug === tag.slug), 1)
            break
          case 'issueType':
            this.issueType.splice(this.issueType.findIndex(item => item.slug === tag.slug), 1)
            break
          case 'status':
            this.status.splice(this.status.findIndex(item => item.slug === tag.slug), 1)
            break
        }
      } else {
        indexOfTag = this.configFilters.findIndex(item => item.configFilterType === configFilterType && item.name === tag.name && item.slug === tag.slug)
      }
